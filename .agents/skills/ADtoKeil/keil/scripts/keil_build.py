...
def parse_log(log_path: str) -> dict:
    ...
    c51_size_match = re.search(
        r"Program Size:\s*data=(\d+(?:\.\d+)?)\s+xdata=(\d+(?:\.\d+)?)(?:\s+const=(\d+(?:\.\d+)?))?\s+code=(\d+(?:\.\d+)?)",
        content,
        re.IGNORECASE,
    )
    if c51_size_match:
        metrics["ram_bytes"] = int(float(c51_size_match.group(1)) + float(c51_size_match.group(2)))
        const_size = float(c51_size_match.group(3) or 0)
        metrics["flash_bytes"] = int(float(c51_size_match.group(4)) + const_size)
...
def _collect_target_artifacts(project_path: Path, target: str) -> dict:
    if project_path.suffix.lower() not in (".uvprojx", ".uvproj"):
        return {}
    ...
    candidates = {
        ...
        "omf_file": output_dir / f"{output_name}.omf",
        "map_file": output_dir / f"{output_name}.m51",
        "lst_file": output_dir / f"{output_name}.lst",
    }
    ...
    debug_file = details.get("elf_file") or details.get("axf_file") or details.get("omf_file")
...
def main() -> None:
    ...
    if raw_result["status"] == "error":
        ...
        artifacts = build_artifacts(
            ...
            omf_file=details.get("omf_file"),
            map_file=details.get("map_file"),
            lst_file=details.get("lst_file"),
